...
/**
 * <p>This class models a Continuous defuzzifier (a defuzzifier for
 * continuous {@link MembershipFunction}s </p>
...
public abstract class DefuzzifierContinuous extends Defuzzifier {
    ...
    public DefuzzifierContinuous(double universeMin, double universeMax) {
        super(universeMin,universeMax);
        init(universeMin, universeMax, DEFAULT_NUMBER_OF_POINTS);
    }

    /** Deffuzification function */
    public abstract double defuzzify();
    public abstract DefuzzifierContinuous newDefuzzifierContinuous();

    public Defuzzifier aggregate(Defuzzifier deffuzifier) {
        try {
            DefuzzifierContinuous deffuzifierAux = (DefuzzifierContinuous)deffuzifier;
            ...
            DefuzzifierContinuous deffuzifierContinuous = newDefuzzifierContinuous();
            int ratioLength;
    ...
    protected void init(double min, double max, int numberOfPoints) {
        ...
        // Check parameters
        if(min >= max)
            throw new RuntimeException("Parameter max is out of range (should satisfy: min < max). min: " + min + "\tmax: " + max);
