...
public class LFTM_Ant {
    ...
    protected Vector<LFTM_Sensor> solution;
    /** Collection of visited servers by this ant */
    protected Vector<LFTM_Sensor> visitedSensors;
     ...
     * @param searchingService The service desired by the client and searched by the LFTM_Ant
     * @param client The client asking for the service
     */
    public LFTM_Ant(Service searchingService, LFTM_Sensor client) {
    ...
    public void addSensor(LFTM_Sensor sensor) {
        LFTM_Sensor currentSensor = getCurrentSensor();
    ...
    public LFTM_Sensor moveBack() {
        ...
        while (solution.size() > 1) {
            ...
            LFTM_Sensor currentNode = getCurrentSensor();
     ...
     */
    private LFTM_Sensor getCurrentSensor() {
     ...
     */
    public void reset() {
        solution = new Vector<LFTM_Sensor>();
        solution.addElement(client);
        visitedSensors = new Vector<LFTM_Sensor>();
    ...
    public double getLastNodePheromone() {
        ...
        if (solution.size() > 2) {
            LFTM_Sensor n1 = solution.lastElement();
            LFTM_Sensor n2 = solution.get(solution.lastIndexOf(n1)-1);
     ...
     */
    public LFTM_Sensor getNextSensor(double alpha, double beta, double q0) {
        LFTM_Sensor nextSensor;
        if (Math.random() <= q0)
     ...
     */
    private LFTM_Sensor leadingExploration(double alpha, double beta) {
        LFTM_Sensor currentNode = getCurrentSensor();
     ...
     * @return The selected sensor according to the formula described above
     */
    private LFTM_Sensor pseudoRandomProportionalRule(double alpha, double beta) {
        LFTM_Sensor currentNode = getCurrentSensor();
    ...
    public void pheromoneLocalUpdating(double phi, Sensor nextSensor) {
        LFTM_Sensor sensor = getCurrentSensor();
    ...
    public void pheromoneGlobalUpdating(double phi, double rho, double maxGlobal) {
        for (int i = 0; i < solution.size()-1; i++) {
            LFTM_Sensor currentServer = solution.get(i);
            LFTM_Sensor nextServer = solution.get(i+1);
     ...
     */
    public Vector<LFTM_Sensor> get_solution() { return solution; }
